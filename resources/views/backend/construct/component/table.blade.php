            ...
            @foreach($constructs as $construct)
                <tr>
                    ...
                    {{-- <td class="text-center js-switch-{{ $construct->id }}">
                        <input type="checkbox" value="{{ $construct->publish }}" class="js-switch status " data-field="publish"
                            data-model="{{ $config['model'] }}" {{ ($construct->publish == 2) ? 'checked' : '' }}
                        data-modelId="{{ $construct->id }}" />
                    </td> --}}
                    <td class="text-center">
                        <a href="{{ route('construction.edit', [$construct->id, $queryUrl ?? '']) }}" class="btn btn-success"><i
                                class="fa fa-edit"></i></a>
                        <a href="{{ route('construction.delete', $construct->id) }}" class="btn btn-danger"><i
