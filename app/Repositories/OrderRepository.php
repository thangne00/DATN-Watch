    ...
    public function getProductReportTime($startDate, $endDate)
{
    return $this->model->select(
        ...
        DB::raw("SUM(order_product.price * order_product.qty) as sum_revenue"),
        // Cách 1: Tính discount theo tỷ lệ sản phẩm trong đơn hàng
        DB::raw("SUM(
            CASE 
                WHEN JSON_UNQUOTE(JSON_EXTRACT(orders.promotion, '$.discount')) IS NOT NULL 
                THEN (order_product.price * order_product.qty) * 
                     (JSON_UNQUOTE(JSON_EXTRACT(orders.promotion, '$.discount')) / 
                      JSON_UNQUOTE(JSON_EXTRACT(orders.cart, '$.cartTotal')))
                ELSE 0 
            END
        ) as sum_discount")
    )
        ...
        ->where('orders.payment', '=', 'paid')
        ->groupBy('order_product.product_id', 'sku', 'product_name') // Thêm group by đầy đủ
        ->get()->toArray();
}
    ...
    public function findByCodeAndCustomer($code, $customerId)
    {
    ...
    public function updateByCode($code, array $attributes)
    {
